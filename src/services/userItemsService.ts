...
export const userItemsService = {
  async getDestinos(userId: number, params?: FetchParams) {
    const response = await api.get<ApiResponse<Destino>>(`/destinos/usuario/${userId}`, { params });
    return response.data;
  },

  async getPasseios(userId: number, params?: FetchParams) {
    const response = await api.get<ApiResponse<Passeio>>(`/passeios/usuario/${userId}`, { params });
    return response.data;
  },

  async getRoteiros(userId: number, params?: FetchParams) {
    const response = await api.get<ApiResponse<Roteiro>>(`/roteiros/usuario/${userId}`, { params });
    return response.data;
  },
